...
def get_accel_distance(Vi, Vf, A_avg, Ai=0.0):
    if math.fabs(A_avg) < 0.01:
        ...
        rospy.loginfo("very low acceleration {} used in get_accel_distance"
                      .format(A_avg))
...
class WaypointUpdater(object):
    def __init__(self):
        ...
        self.waypoints = []
        self.pose = None
        self.last_pose_time = 0  # monitor time we receive fresh pose updates
        self.velocity = None
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            if self.waypoints and self.waypoints_2d and self.waypoint_tree:
                self.send_waypoints()
                loop_counter += 1
                if rospy.get_time() - loop_time > 1:
                    if loop_counter < 10:
                        rospy.loginfo("Only ran {} loops in last second".format(loop_counter))
                    loop_time = rospy.get_time()
                    loop_counter = 0
            self.rate.sleep()
        ...
        if old_buffer_offset != config['dyn_buffer_offset']:
            ...
            if self.dyn_buffer_offset > self.dyn_tl_buffer:
                rospy.logwarn("dyn_buffer_offset={:3.2f}m set larger"
                              "than dyn_tl_buffer={:3.2f}m".format(
        ...
        if traffic_msg.data != self.next_tl_wp_tmp:
            if self.dyn_test_stoplight is False:
                ...
                if self.next_tl_wp_tmp == -1:
                    ...
                    tmp_dist_to_tl = self.waypoints[self.next_tl_wp_tmp].get_s() -\
                        self.waypoints[self.final_waypoints_start_ptr].get_s() - self.dyn_tl_buffer
                    rospy.logwarn("new /traffic_waypoint message received is wp: {}, at distance of {:3.2f}m "
                                  "from car at wp {}".format(self.next_tl_wp_tmp, tmp_dist_to_tl,
                                  self.final_waypoints_start_ptr))
            else:
    ...
    def set_stopped(self, start_ptr, num_wps):
        ...
        if self.state != 'stopped':
            rospy.logwarn("Set car state to stopped at ptr = {}, distance to tl = {:3.3f}".format(start_ptr, self.get_distance_to_target()))
            self.state = 'stopped'
    ...
    def produce_slowdown(self, start_ptr, num_wps, distance):
        ...
        if curpt.JMT_ptr == -1 or self.state != 'slowdown':
            ...
            if self.state != 'slowdown':
                rospy.logwarn("Set car state to slowdown at ptr = {}, stop_target = {} - {:3.2f}m".format(curpt.ptr_id, self.next_tl_wp, (self.dyn_tl_buffer - self.dyn_buffer_offset)))
                self.state = 'slowdown'
        else:
    ...
    def set_waypoints_velocity(self):
        ...
        if self.waypoints[self.final_waypoints_start_ptr].get_v() == 0.0:
            ...
            if self.velocity > 0.05:
                rospy.loginfo("Car state is stopped, but car is moving at velocity = {:3.2f} at ptr = {}"
                              .format(self.velocity, self.final_waypoints_start_ptr))
        else:
            pose_delay = rospy.get_time() - self.last_pose_time
            if pose_delay > 0.1:
                rospy.loginfo("Delay of {:3.3f} s since pose update while car is moving".format(pose_delay))
        ...
        # switch to slowdown if possible!
            ...
            if dist_to_tl - self.dyn_tl_buffer > self.min_stop_distance:
                if self.state != 'slowdown':
                    rospy.logwarn("TL at {:3.2f}m is within stopping distance of {:3.2f}m"
                                  " - Start to slowdown!".format(dist_to_tl - self.dyn_tl_buffer, self.stopping_distance))
                self.stop_target = self.next_tl_wp
                recalc = self.produce_slowdown(self.final_waypoints_start_ptr,
                            self.lookahead_wps,
                            dist_to_tl - (self.dyn_tl_buffer - self.dyn_buffer_offset))
        ...
        if recalc is True:
            rospy.loginfo("recalc is set to {} we should recalculate"
                          .format(recalc))
    ...
    def setup_speedup_jmt(self, curpt, a_dist, target_velocity, acceltime=0):
        ...
        if a_dist < 0.1 or T < 0.1:
            # dummy values to prevent matrix singularity
            # if no velocity change required
            rospy.loginfo("No change in velocity needed.")
            a_dist = 0.1
            T = 0.1
    ...
    def get_max_accel(self, ptr_id):
        ...
        if (self.default_velocity - curpt.get_v()) < 0.2 and curpt.get_a() >= 0.0:
            rospy.loginfo("no big difference between velocities- just switch to desired at next wp")
            return (0.5, 0.5, 0.1)
        ...
        while optimized is False:
            ...
            if counter > 30:
                rospy.loginfo("counter is {} in get_max_accel - bail with acc={:3.2f}!"
    ...
    def get_speedup_time(self, start, end):
        ...
        if start[1] >= self.default_velocity * 0.95 and\
            start[2] >= 0.0:
            rospy.loginfo("Only small velocity change, time of 0.5 returned")
            return 0.5
        ...
        while optimized is False:
            ...
            if e_acc > end[2] and s_acc > start[2]:
                ...
                if counter > 40 or T < Tmin:
                    rospy.loginfo("counter is {} T= {:3.2f} in get_speedup_time - bail on searching in this direction!"
                                .format(counter, T))
                    optimized = True
                    try_reverse = True
                rospy.logdebug("found initial jerk of {:3.2f}m/s^3 and start_acc={:3.3f}m/s^2 end_acc={:3.3f}m/s^2"
        ...
        while optimized is False:
            ...
            if e_acc > end[2] and s_acc > start[2]:
                ...
                if counter > 30 or T < Tmin:
                    rospy.loginfo("counter is {} in get_speedup_time - bail!"
                                .format(counter))
                    optimized = True
                rospy.logdebug("found initial jerk of {:3.2f}m/s^3 and start_acc={:3.3f}m/s^2 end_acc={:3.3f}m/s^2"
                            " using a_dist of {:3.2f}m and T={:3.2f}s"
    ...
    def get_distance_to_target(self):
        ...
        x1 = self.waypoints[self.stop_target].get_x()
        y1 = self.waypoints[self.stop_target].get_y()
        ...
        if self.waypoints and self.last_search_distance:
            ...
            for i in range(self.final_waypoints_start_ptr, min(self.
                ...
                if tmpdist < dist:
                    ...
                    rospy.loginfo("i = {}, new dist {:4.3f} - {:4.3f} > 5"
                                  .format(i, dist, self.last_search_distance))

            # fall out no closest match that looks acceptable
            if found is False:
                rospy.loginfo("waypoint_updater:closest_waypoint local search not "
    ...
    def run_tests(self):
        ...
        ## Test for Track 1
        ## test of creep
        if self.final_waypoints_start_ptr in range(270,292):
            self.next_tl_wp = 293
            self.stop_target = 293
            if self.state != 'creeping' and self.final_waypoints_start_ptr >= 288:
                self.test_counter += 1
                if self.test_counter == 60:
                    self.test_time = rospy.get_time()
                if self.test_counter >= 60:
                    self.next_tl_wp = -1
        
        if self.final_waypoints_start_ptr in range(292,300):
            if rospy.get_time() - self.test_time > 3.5:
                self.next_tl_wp = 293
                self.stop_target = 293
            self.test_counter = 0
        ## Test of seeing light too late
        if self.final_waypoints_start_ptr >= 300 and self.final_waypoints_start_ptr < 310:
            self.next_tl_wp = 307
            if self.state != 'slowdown' and self.final_waypoints_start_ptr >= 304:
                self.test_counter += 1
        ...
        if self.final_waypoints_start_ptr > 450 and self.final_waypoints_start_ptr < 480:
            self.test_counter = 0
            self.next_tl_wp = 500
        if self.final_waypoints_start_ptr >= 510 and self.final_waypoints_start_ptr < 754:
            self.next_tl_wp = 753
            if self.state != 'slowdown' and self.final_waypoints_start_ptr >= 751:
                self.test_counter += 1
                if self.test_counter > 60:
                    self.next_tl_wp = -1
